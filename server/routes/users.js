...
router.put('/:username', function(req, res) {
  ...
  User.findOne({ username: req.params.username }).then(function(user){
    ...
    if (Object.keys(userUpdateDict).length > 0) {
        User.findOneAndUpdate({ username: req.params.username}, { "$set": {...userUpdateDict}}).then(function(){
    ...
    if (Object.keys(profileUpdateDict).length > 0) {
      Profile.findOneAndUpdate({ profileID: req.params.username}, { "$set": {...profileUpdateDict}}, { upsert: true }).then(function(){
        // res.status(200).send(messageDict);
